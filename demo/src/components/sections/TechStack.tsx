      ...
      <div className="mt-8 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
        <div className="flex w-max animate-marquee gap-4">
          {loop.map((tech, i) => (
            <span
              key={`${tech}-${i}`}
              className="flex items-center rounded-full border border-white/10 bg-white/[0.03] px-6 py-2.5 text-sm font-medium text-ink-300 transition-colors hover:border-brand-400/30 hover:text-white"
            >
              {tech}
            </span>
          ))}
